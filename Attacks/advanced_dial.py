...
def generate_attack(device,tokenizer,seed,model_input,num_topics,chat_history_ids,with_toxicity,less_toxic):
	...
	for topic in range(num_topics):
		adv_list = run_model(model_input,1,topic,seed)
		for adv in adv_list:
			...
			if (with_toxicity ==1 and less_toxic==1):
				...
				if("attributeScores" in results):
					if (results['attributeScores']['TOXICITY']['summaryScore']['value'] < 0.5):
						candidate_response_scores.append(results['attributeScores']['TOXICITY']['summaryScore']['value'])
	...
	return candidate_responses,candidate_length,candidate_response_scores, temp_chat_history_ids, new_bot_input_ids
...
if __name__ == '__main__':
	...
	for step in range(5):
		if step == adv_step:
			candidate_responses,candidate_length,candidate_response_scores, temp_chat_history_ids, new_bot_input_ids = generate_attack(device,tokenizer,int(args.seed),model_input,num_topics,chat_history_ids,with_toxicity,less_toxic)			
