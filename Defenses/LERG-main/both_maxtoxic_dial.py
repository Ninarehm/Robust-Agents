		...
		for adv in adv_list:
			...
			if("attributeScores" in results):
				if(parl_results[0]==False):
					candidate_response_scores.append(  ( (float(results['attributeScores']['TOXICITY']['summaryScore']['value'])  +float(-1*parl_results[1])  )/2 ) )
				elif(parl_results[0]==True):
					candidate_response_scores.append(  ( (float(results['attributeScores']['TOXICITY']['summaryScore']['value'])  +float(parl_results[1])  )/2) )
