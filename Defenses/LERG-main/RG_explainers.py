...
class LERG_SHAP_log(LERG):
    ...
    def get_local_exp(self):
        ...
        phi_sets = []
        for i in range(len(self.x)):
            x_set, x_set_with_i, weights, self.components = self.perturb_inputs(self.x, num=500//len(self.x), with_i=i)# results in total 1000 samples as LERG_LIME

            
            #########################Nina Changed###############################
            nina_x_set = [self.tokenizer.convert_tokens_to_ids(x) for x in x_set] 
            nina_x_set = [[self.tokenizer.convert_tokens_to_ids("<speaker1>")] + x for x in nina_x_set]
            max_l = max(len(x) for x in nina_x_set)
            nina_x_set = [x + [self.tokenizer.convert_tokens_to_ids("<pad>")] * (max_l - len(x)) for x in nina_x_set]
            nina_y = self.tokenizer.convert_tokens_to_ids(self.y) 
            nina_y = [self.tokenizer.convert_tokens_to_ids("<speaker2>")] + nina_y
        
            input_ids = torch.tensor([[self.tokenizer.convert_tokens_to_ids("<bos>")] + x+nina_y for x in nina_x_set]).to(self.device)
            token_type_ids = torch.tensor([[self.tokenizer.convert_tokens_to_ids("<speaker1>")] * (len(x)+1) + [self.tokenizer.convert_tokens_to_ids("<speaker2>")] * len(nina_y) for x in nina_x_set]).to(self.device)


            
            outputs = self.model_f(input_ids,token_type_ids=token_type_ids)
            nina_probs = F.softmax(outputs.logits, dim=-1)
            probs = nina_probs[:,-len(nina_y):-1,:]
            y = nina_y[1:]
            #########################Nina Changed###############################
            y_probs = self.get_prob(probs, y)
            y_probs = torch.tensor(y_probs)
            ...
            #########################Nina Changed###############################
            nina_x_set = [self.tokenizer.convert_tokens_to_ids(x) for x in x_set_with_i] 
            nina_x_set = [[self.tokenizer.convert_tokens_to_ids("<speaker1>")] + x for x in nina_x_set]
            max_l = max(len(x) for x in nina_x_set)
            nina_x_set = [x + [self.tokenizer.convert_tokens_to_ids("<pad>")] * (max_l - len(x)) for x in nina_x_set]
            nina_y = self.tokenizer.convert_tokens_to_ids(self.y) 
