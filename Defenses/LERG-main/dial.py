...
def dialouge_interpretability(temp_defender_chat_history_ids,adv_starting_idx,bot_input_ids,attentive_token):
	...
	if temp_defender_chat_history_ids.shape[1]==68 and bot_input_ids.shape[-1]==42:
		local_exp  = LERG_S(model, tokenizer.decode(temp_defender_chat_history_ids[:,adv_starting_idx: bot_input_ids.shape[-1]][0], skip_special_tokens=True), tokenizer.decode(temp_defender_chat_history_ids[:, bot_input_ids.shape[-1]:bot_input_ids.shape[-1]+21][0], skip_special_tokens=True), perturb_f, tokenizer)
	else:
...
if __name__ == '__main__':
	...
	for step in range(5):
		if step == adv_step:
			adv = run_model(model_input,int(args.loss_type))
			adv = tokenizer.encode(adv, return_tensors='pt')
			adv = adv.to(device)
			if step ==0:
				chat_history_ids =adv
				new_bot_input_ids = 0
			elif  step != 0:
				new_bot_input_ids = chat_history_ids
				chat_history_ids = torch.cat([chat_history_ids,adv[:,:]], dim=-1)
				

			#chat_history_ids = model.generate(chat_history_ids, max_length=1000, pad_token_id=tokenizer.eos_token_id,do_sample=True,top_p=0.95,top_k=30)
		...
		if (step == adv_step):
			...
			old_attn_mask = torch.ones_like(chat_history_ids)
			new_expt_i = 0
			new_expt_counter = 0
			while True:
				new_expt_counter+=1
				if HF_result =="toxic":
					...
					attn_mask  = old_attn_mask*attn_mask
					temp_defender_chat_history_ids = model.generate(bot_input_ids, max_length=1000, pad_token_id=tokenizer.eos_token_id,attention_mask=attn_mask,do_sample=True,top_p=0.95,top_k=30)
					HF_result = HF_is_toxic(cls_explainer,tokenizer.decode(temp_defender_chat_history_ids[:, bot_input_ids.shape[-1]:][0], skip_special_tokens=True))
					old_attn_mask = attn_mask
					new_expt_i +=1
				else:
					chat_history_ids = temp_defender_chat_history_ids
					break
				if new_expt_counter > 100000:
					chat_history_ids = temp_defender_chat_history_ids
					break
		...
		if chat_history_ids.shape[1] > 100:
			...
			print("step before 100 word" + str(step))
