	...
	for topic in range(num_topics):
		...
		for adv in adv_list:
			...
			if (with_toxicity ==1 and less_toxic==1):
				...
				results = HF_is_toxic(cls_explainer,temp_text)
				if (results =="not"):
...
if __name__ == '__main__':
	...
	for step in range(5):
		if step == adv_step:
			candidate_responses,candidate_length,candidate_response_scores, temp_chat_history_ids, new_bot_input_ids = generate_attack(device,tokenizer,int(args.seed),model_input,num_topics,chat_history_ids,with_toxicity,less_toxic,cls_explainer)			
