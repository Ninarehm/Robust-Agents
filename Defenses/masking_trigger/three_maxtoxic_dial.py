	...
	for topic in range(num_topics):
		...
		for adv in adv_list:
			...
			if("attributeScores" in results):
				if(parl_results[0]==False):
					candidate_response_scores.append(  ( (float(results['attributeScores']['TOXICITY']['summaryScore']['value'])  +float(-1*parl_results[1])+temp_result_score  )/3 ) )
				elif(parl_results[0]==True):
					candidate_response_scores.append(  ( (float(results['attributeScores']['TOXICITY']['summaryScore']['value'])  +float(parl_results[1]) +temp_result_score )/3) )
...
if __name__ == '__main__':
	...
	for step in range(5):
		if step == adv_step:
			candidate_responses,candidate_response_scores, temp_chat_history_ids, new_bot_input_ids,offensive_classifier_multi,offensive_classifier_single = generate_attack(device,tokenizer,int(args.seed),model_input,num_topics,chat_history_ids,with_toxicity,less_toxic,cls_explainer)			
