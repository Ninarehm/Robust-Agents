	...
	for topic in range(num_topics):
		...
		for adv in adv_list:
			...
			if("attributeScores" in results):
				candidate_response_scores.append(float(results['attributeScores']['TOXICITY']['summaryScore']['value']) )
...
if __name__ == '__main__':
	...
	for step in range(5):
		if step == adv_step:
			candidate_responses,candidate_response_scores, temp_chat_history_ids, new_bot_input_ids = generate_attack(device,tokenizer,int(args.seed),model_input,num_topics,chat_history_ids,with_toxicity,less_toxic)			
